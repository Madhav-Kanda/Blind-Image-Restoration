...
def train_fn(
    ...
    for idx, (Image_blur, Image_sharp) in enumerate(
        loader
    ):  ## Iterating over all the images in the train loader
        ...
        with torch.cuda.amp.autocast():
            ...
            L1_LPIPS = torch.mean(
                l1_loss(y_fake, Image_sharp) * 170
                + lpips_loss(y_fake, Image_sharp) * 145
            )
        ...
        if idx % 50 == 0:  ## After feeding every 50 images printing the losses
            print(
                "[%d/%d][%d/%d]\tD_loss: %.4f\tG_loss: %.4f"
                % (epoch, epochs, idx, len(loader), D_loss.item(), G_loss.item())
            )
...
def main():
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=8, shuffle=True, num_workers=16, pin_memory=True
    )
    ...
    for epoch in range(
        NUM_EPOCHS
    ):  ## Runing for the specified number of epochs in config variables
        ...
        if (
            SAVE_MODEL and epoch % 10 == 0
        ):  ## Saving the model checkpoint after every 10 iterations
            save_checkpoint(
                gen, opt_gen, filename=f"checkpoints/gen/gen.pth_{epoch}.tar"
            )
            save_checkpoint(
                disc, opt_disc, filename=f"checkpoints/disc/disc_{epoch}.pth.tar"
            )

        save_some_examples(
            gen, train_loader, epoch, folder="results/train"
    ...
    for idx, (Image_blur, Image_sharp) in enumerate(val_loop):
        val_loader = save_val_examples(
            Image_blur, Image_sharp, gen, idx, folder="results/val"
        )
